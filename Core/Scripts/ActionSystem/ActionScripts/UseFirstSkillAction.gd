...
func start_action(target_package: TargetPackage = null) -> void:
	...
	var explicit_skill: Skill = null
	if target_package != null:
		explicit_skill = target_package.get_skill()

	# If combat is live, delegate to TurnSystem so it handles AP, chains, and logging.
	if TurnSystem.instance != null and TurnSystem.instance.current_phase != TurnSystem.RoundPhase.PLAYER_PLANNING:
		var _ok: bool = await TurnSystem.instance.execute_active_skill_for_unit(unit, explicit_skill)
	else:
		# Out-of-combat tester: keep the old behavior.
		var chosen_skill: Skill = explicit_skill
		if chosen_skill == null and unit != null and unit.tactics_controller != null:
...
func start_action_dep(target_package: TargetPackage = null) -> void:
